    ...
    public class MovingTransition : BaseTransition
    {
        ...
        public override void TriggerTransition(bool forceReset)
        {
            if(transitionTransform == null)
                transitionTransform = parent.transform;
        ...
        public override void TriggerFadeOut()
        {
            ...
            if(endAtCurrent)//if we need to update the end point to ensure we start smoothly
                ...
                {
                    case MovementType.Absolute:
                        endPoint = transitionTransform.position;//then update it with the current position
                        break;
        ...
        protected override void Transition(float transitionPercentage)
        {
            ...
            {
                case MovementType.Absolute:
                    transitionTransform.position = startPoint + (endPoint - startPoint) * transitionPercentage;
                    break;
                ...
                case MovementType.Local:
                    transitionTransform.localPosition = startPoint + (endPoint - startPoint) * transitionPercentage;
                    break;

                case MovementType.RectTransform:
                    (transitionTransform as RectTransform).anchoredPosition3D = startPoint + (endPoint - startPoint) * transitionPercentage;
                    break;
            }
        }
